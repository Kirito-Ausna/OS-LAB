...
* 本次实验中仅需要编写SJF下的进程调度，由于测试方式更新，需要删除`task_test_done`函数，如果有其他未提及的修改可以备注在附录中。

#### 4.4.2 修改task_init()调整为虚拟地址
...
开启虚拟内存后，会存在一些断点失效的情况，GDB无法在C语言源代码层面调试，只能在汇编等级调试。原因是开启MMU后，PC在虚拟的0x80000000空间下，但是GDB中记录的调试信息是在0xfffffe000000000空间下的，因此无法获取调试信息。
